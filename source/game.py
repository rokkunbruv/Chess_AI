    ...
    def show_pieces(self, surface, board):
        for piece in board.pieces_on_board:
            if piece is not self.dragger.piece:
                ...
                image_center = piece.pos[0] * TILE_SIZE + TILE_SIZE // 2, piece.pos[1] * TILE_SIZE + TILE_SIZE // 2
                piece.image_rect = image.get_rect(center=image_center)
                surface.blit(image, piece.image_rect)

        '''for row in range(ROWS):
            for col in range(COLS):
                # draws the piece if it's on the tile
                if self.board.tiles[row][col].has_piece():
                    piece = self.board.tiles[row][col].piece

                    # ensures that the piece being drawn is not the piece that is dragged (larger image)
                    if piece is not self.dragger.piece:
                        piece.set_image(size = PIECE_SIZE)
                        ...
                        image_center = col * TILE_SIZE + TILE_SIZE // 2, row * TILE_SIZE + TILE_SIZE // 2
                        piece.image_rect = image.get_rect(center=image_center)
                        surface.blit(image, piece.image_rect)'''
    ...
    def show_moves(self, surface):
        ...
        if self.dragger.dragging:
            ...
            for move in piece.moves:
                ...
                pygame.draw.rect(surface, color, [move.final.col * TILE_SIZE, move.final.row * TILE_SIZE, TILE_SIZE, TILE_SIZE])
    ...
    def show_last_moves(self, surface):
        ...
        if self.board.last_move:
            ...
            for pos in [initial, final]:
                ...
                pygame.draw.rect(surface, color, [pos.col * TILE_SIZE, pos.row * TILE_SIZE, TILE_SIZE, TILE_SIZE])
